...
class DFRobot_Gesture {
  ...
  void     begin(unsigned char pin_xfer, unsigned char pin_reset),
           onTouch( void (*)(unsigned char) ),
           onAirwheel( void (*)(int) ),
           onGesture( void (*)(unsigned char) ),
           onXYZ( void (*)(unsigned int, unsigned int, unsigned int) );
  int      poll();
  bool     onGesture(GESTURE gesture);

  unsigned char last_gesture, 
                last_touch;
  unsigned int  x,
                y,
                z;
  ...
  int handle_sensor_data(unsigned char* data);

  void addEvent(GESTURE gesture);
  void recordEvents(GESTURE gesture);
  bool extractEvent(GESTURE gesture);
};
