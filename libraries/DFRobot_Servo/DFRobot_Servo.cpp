...
uint8_t Servo::attach(int pin)
{
    ...
    _maxUs = max((uint16_t)250, min((uint16_t)3000, _maxUs));
    _minUs = max((uint16_t)200, min(_maxUs, _minUs));
    angle(_valueUs);
    return pin;
#elif defined(ARDUINO_ARCH_RP2040)
    return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH, 1500);
#endif
}
#if defined(__AVR__)
...
#elif defined(ARDUINO_ARCH_RP2040)
int Servo::attach(int pin, int minUs, int maxUs, int value) {
