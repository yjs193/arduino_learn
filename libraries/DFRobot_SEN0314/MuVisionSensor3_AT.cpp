...
uint8_t MuVisionSensor3_AT::WifiSet(String ssid, String password, String apmode) {
  writeStr(MU_AT_START "+" MU_AT_WIFISET "=");
  writeStr(ssid.c_str());
  writeByte(MU_AT_PARAM_SEPARATOR);
  writeStr(password.c_str());
  writeByte(MU_AT_PARAM_SEPARATOR);
  writeStr(apmode.c_str());
  writeStr(MU_AT_END_LINE);
  char buf[MU_BUF_DEEP+20] = {0};
  readUntil('\n', buf, sizeof(buf));
  if (!strcmp(buf, MU_AT_OK)) {
...
uint8_t MuVisionSensor3_AT::WifiUDP(String ip, String port) {
