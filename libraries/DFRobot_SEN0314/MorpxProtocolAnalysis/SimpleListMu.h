...
/*
 * @brief Node for class SimpleListMu.
 * @see SimpleListMu
 */
...
class SimpleListMu {
 public:
  ...
  SimpleListMu() {}
  ~SimpleListMu() {}
  SimpleListMu& operator=(const SimpleListMu& list) {
    this->size_ = list.size_;
    this->head_ = list.head_;
    return *this;
  }
  SimpleListMu(const SimpleListMu& list) {
    this->size_ = list.size_;
    this->head_ = list.head_;
  }
...
class SimpleQueue : public SimpleListMu<node_t<element_t>> {
 public:
  SimpleQueue() {}
  ~SimpleQueue() {}
   ...
   * @see size()
   */
  element_t& peek(void) { return SimpleListMu<node_t<element_t>>::front()->element_; };
  /*
   ...
   */
  element_t& pop(void) {
    node_t<element_t>* node = SimpleListMu<node_t<element_t>>::pop_front();
    if (node) {
     first_++;
     if (first_ >= DEEP) {
       first_ = 0;
  ...
  element_t& push(const element_t& elem) {
    if (SimpleListMu<node_t<element_t>>::size() >= DEEP) {
      pop();
    }
    memcpy(&node_[last_].element_, &elem, sizeof(element_t));
    SimpleListMu<node_t<element_t>>::push_back(&node_[last_]);
    last_++;
    if (last_ >= DEEP) {
      last_ = 0;
    }
  ...
  void showMessage() {
    printf("first_ = %u, last_ = %u, size = %u\n", first_, last_, SimpleListMu<node_t<element_t>>::size());
  }
  void clear(void) {
    SimpleListMu<node_t<element_t>>::clear();
    first_ = 0;
    last_ = 0;
  }
...
//SimpleListMu<element_t>::SimpleListMu() {
//}
//
//template<typename element_t>
//SimpleListMu<element_t>::~SimpleListMu() {
...
//    SimpleNode<int>* elem;
//    SimpleListMu<SimpleNode<int>> list;
