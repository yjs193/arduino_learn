...
{
public:
    uint8_t    receive_ACK[35];// 串口应答数据缓存
    uint8_t    passWord[6]; // 块密码，出厂默认为 0xff 0xff 0xff 0xff 0xff 0xff
    uint8_t    uid[4]; // 读取uid缓存
...
private:
    ...
    bool          passWordCheck (int blockNumber,uint8_t uid[],  uint8_t keyData[]);
    bool          checkDCS(int x);
