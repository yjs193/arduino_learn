...
#include <DFRobot_DHT20_I2C.h>


DFRobot_DHT20_I2C::DFRobot_DHT20_I2C(TwoWire * pWire,uint8_t address)
  : _pWire(pWire) {
  _address = address;
}

int DFRobot_DHT20_I2C::begin() {
...
float DFRobot_DHT20_I2C::getTemperature() {
...
float DFRobot_DHT20_I2C::getHumidity() {
  uint8_t readCMD[3]={0xac,0x33,0x00};
  uint8_t data[6] = {0};
  int retries = 10;
  float humidity;
...
void DFRobot_DHT20_I2C::writeCommand(const void *pBuf, size_t size) {
  if (pBuf == NULL) {
...
uint8_t DFRobot_DHT20_I2C::readData(void *pBuf, size_t size) {
  delay(10);
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
