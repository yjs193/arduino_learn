...
void DFRobot_BMI160::begin(TYPE v, uint8_t addr){
  Wire.begin();
	I2cInit(addr); 
	if(v == eStep){
        setInt(2); 
        setStepCounter();  
        setStepPowerMode(stepNormalPowerMode);
	}
}

float DFRobot_BMI160::getX(){
	int rslt;
	float k;
...
float DFRobot_BMI160::getY(){
	int rslt;
	float k;
	int16_t accelGyro[6]={0};
	rslt =getAccelGyroData(accelGyro);
	k=accelGyro[1]*3.14/180.0;
	return k;
}
float DFRobot_BMI160::getZ(){
	int rslt;
	float k;
	int16_t accelGyro[6]={0};
	rslt =getAccelGyroData(accelGyro);
	rslt =getAccelGyroData(accelGyro);
	k=accelGyro[2]*3.14/180.0;
	return k;
}
float DFRobot_BMI160::getAccX(){
	int rslt;
	float k;
	int16_t accelGyro[6]={0};
	rslt =getAccelGyroData(accelGyro);
...
int DFRobot_BMI160::getstep(void){
	uint16_t k=0;
    uint16_t stepCounter = 0;
    //read step counter from hardware bmi160 
    if (readStepCounter(&stepCounter)==BMI160_OK){
	  k=stepCounter;
    }
	return k;
}
